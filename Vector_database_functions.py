...
def Create(docs, collection_name, embedding_function, data_directory):
    ...
    Chroma.from_documents(docs, embedding_function, client=chroma_client, collection_name=collection_name, persist_directory=data_directory)
...
def get_db(collection_name, data_directory, embedding_function):
    db = Chroma(persist_directory = data_directory, collection_name=collection_name, client=chroma_client, embedding_function = embedding_function)
    return db
